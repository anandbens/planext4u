CREATE OR REPLACE FUNCTION public.validate_coupon_code(_code text, _customer_id text, _cart_items jsonb, _subtotal numeric, _lat double precision DEFAULT NULL::double precision, _lng double precision DEFAULT NULL::double precision)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  IF array_length(camp.district_ids, 1) IS NOT NULL AND array_length(camp.district_ids, 1) > 0 THEN
    -- FIX: districts.id is uuid, campaigns.district_ids is text[] — compare as text
    SELECT EXISTS (
      ...
      WHERE ca.customer_id = _customer_id
        AND d.id::text = ANY(camp.district_ids)
    ) INTO addr_match;
  ...
  IF eligible_product IS NULL THEN RETURN jsonb_build_object('valid', false, 'reason', 'No matching product in cart for this coupon'); END IF;
