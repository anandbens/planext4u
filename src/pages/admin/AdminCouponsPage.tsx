...
export default function AdminCouponsPage() {
  ...
  const load = async () => {
    ...
    const [{ data: c }, { data: v }, { data: d }] = await Promise.all([
      ...
      supabase.from("vendors").select("id, business_name, name, state_name, city_id, shop_latitude, shop_longitude, status").in("status", ["active", "verified", "approved"]).order("business_name"),
