...
import { Search, ShoppingCart, User, MapPin, ChevronRight, Star, Heart } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-8">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data?.banners.map((b) => (
              <div key={b.id} className={`bg-gradient-to-r ${b.gradient || 'from-primary to-primary/70'} rounded-2xl p-6 text-primary-foreground`}>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {isLoading
              ...
              : data?.categories.map((c) => (
                <Link to={`/app/browse?category=${c.name}`} key={c.id} className="bg-card rounded-xl border border-border/50 p-4 text-center hover:border-primary/30 transition-colors">
