...
export function SubcategoryStrip({ parentName, subcategories }: SubcategoryStripProps) {
  const [open, setOpen] = useState(false);
  if (!subcategories.length) return null;

  const visible = subcategories.slice(0, 7);
  const hasMore = subcategories.length > visible.length;
